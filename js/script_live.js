$(function() {
  ...
  var App = {
    ...
    initCameraSelection: function() {
      ...
      return Quagga.CameraAccess.enumerateVideoDevices().then(function(devices) {
        ...
        devices.forEach(function(device) {
          ...
          $option.appendChild(document.createTextNode(pruneText(device.label ||
    ...
    attachListeners: function() {
      ...
      $(".controls .reader-config-group").on("change", "input, select", function(e) {
        e.preventDefault();
        var $target = $(e.target),
          value = $target.attr("type") === "checkbox" ? $target.prop("checked") :
          $target.val(),
  ...
  $(".controls").on("click", "button.start_live", function(e) {
    e.preventDefault();
    App.init();
  });
  Quagga.onProcessed(function(result) {
    ...
    if (result) {
      ...
      if (result.box) {
        Quagga.ImageDebug.drawPath(result.box, { x: 0, y: 1 }, drawingCtx, { color: "#00F",
          lineWidth: 2 });
      }
      if (result.codeResult && result.codeResult.code) {
        Quagga.ImageDebug.drawPath(result.line, { x: 'x', y: 'y' }, drawingCtx, { color: 'red',
          lineWidth: 3 });
      }
    }
  });
